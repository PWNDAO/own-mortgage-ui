        ...
        <div class="relative">
            <div class="flex items-center gap-2">
                <Input
                    v-model="lendAmount"
                    placeholder="150.23"
                    :disabled="walletBalance <= 0n || isInputDisabled"
                    class="w-full h-[40px]"
                    :class="{ 'border-red-500': isAmountInvalid }"
                />
                <Button class="h-[40px]" @click="handleMaxClick">
                    Max
                </Button>
            </div>
...
import { CREDIT_DECIMALS, CREDIT_NAME } from '~/constants/proposalConstants'
...
const walletBalanceFormatted = computed(() => formatUnits(walletBalance.value, CREDIT_DECIMALS))
...
const handleMaxClick = () => {
    if (walletBalance.value > missingAmount.value) {
